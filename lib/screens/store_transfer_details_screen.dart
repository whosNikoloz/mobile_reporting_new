...
class StoreTransferDetailsScreen extends StatefulWidget {
  const StoreTransferDetailsScreen({super.key});

  @override
  State<StoreTransferDetailsScreen> createState() =>
      _StoreTransferDetailsScreenState();
}

class _StoreTransferDetailsScreenState
    extends State<StoreTransferDetailsScreen> {
  ...
  List<StoreTransferDetailsResponseModel> _items = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await _reportsService.getStoreTransferDetails(
        startDate: _startDate,
        endDate: _endDate,
        fromStoreId: _fromStoreId ?? 0,
        toStoreId: _toStoreId ?? 0,
        page: _currentPage,
        pageSize: _pageSize,
      );
      setState(() {
  ...
  Future<void> _showStoreDialog(bool isFrom) async {
    ...
    final storeChanged = await showDialog<bool>(
      ...
      builder: (context) => StoreSelectorDialog(
        stores: stores,
        selectedStoreId: isFrom ? _fromStoreId : _toStoreId,
        onStoreSelected: (storeId) {
          setState(() {
            if (isFrom) {
              _fromStoreId = storeId;
            } else {
              _toStoreId = storeId;
            }
          });
  ...
  String _getStoreName(int? storeId) {
    ...
    try {
      return application.stores.firstWhere((store) => store.id == storeId).name;
    } catch (_) {
      return S.of(context).allBranches;
    }
  }
      ...
      body: !_filtersLoaded
          ...
          : Column(
              children: [
                ...
                if (application.stores.length > 1)
                  Padding(
      ...
      child: Row(
        children: [
          Expanded(
            flex: 4,
            child: Text(
              l10n.location, // Mapping "Route" or locations conceptually based on available l10n
              style: const TextStyle(
                fontSize: 13,
                fontWeight: FontWeight.w600,
  ...
  Widget _buildRow(StoreTransferDetailsResponseModel item, int index) {
    final l10n = S.of(context);
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
      decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            flex: 2,
            child: Text(
              CurrencyHelper.format(item.quantity,
                  showSymbol: false, showDecimals: true),
              textAlign: TextAlign.right,
              style: const TextStyle(
                fontSize: 13,
                fontWeight: FontWeight.w500,
