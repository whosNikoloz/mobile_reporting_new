...
class PurchasesDocumentsScreen extends StatefulWidget {
  const PurchasesDocumentsScreen({super.key});

  @override
  State<PurchasesDocumentsScreen> createState() => _PurchasesDocumentsScreenState();
}

class _PurchasesDocumentsScreenState extends State<PurchasesDocumentsScreen> {
  ...
  List<ContragentPaymentsResponseModel> _items = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await _reportsService.getPurchaseDocuments(
  ...
  Widget _buildRow(ContragentPaymentsResponseModel item, int index) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
      decoration: BoxDecoration(
        border: Border(
