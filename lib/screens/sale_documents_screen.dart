...
class SaleDocumentsScreen extends StatefulWidget {
  const SaleDocumentsScreen({super.key});

  @override
  State<SaleDocumentsScreen> createState() => _SaleDocumentsScreenState();
}

class _SaleDocumentsScreenState extends State<SaleDocumentsScreen> {
  ...
  List<SaleDocumentsResponseModel> _items = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await _reportsService.getSaleDocuments(
  ...
  Widget build(BuildContext context) {
    ...
    // Provide localized title
    String screenTitle = "Sale Documents";
    try { screenTitle = l10n.saleDocuments; } catch(_) {}
                                  ...
                                  children: [
                                    ...
                                    ...() {
                                      final filtered = _items
                                          .where((item) => item.contragentName
                                              .toLowerCase()
  ...
  Widget _buildRow(SaleDocumentsResponseModel item, int index) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
      decoration: BoxDecoration(
        border: Border(
      ...
      child: Row(
        ...
        children: [
          Expanded(
            flex: 3,
            child: Text(
              DateFormat('dd.MM.yyyy HH:mm').format(item.tDate),
              maxLines: 2,
