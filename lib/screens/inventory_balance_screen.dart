...
class _InventoryBalanceScreenState extends State<InventoryBalanceScreen> {
  ...
  List<ProductRestResponseModel> _items = [];
  ...
  Future<void> _loadFilters() async {
    ...
    if (mounted) {
      setState(() {
        _date = application.endCurrentPeriod ?? _date;
        _filtersLoaded = true;
      });
    }
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await _reportsService.getInventoryBalance(
        asOfDate: _date,
        page: _currentPage,
        pageSize: _pageSize,
      );
    ...
    return Scaffold(
      ...
      body: !_filtersLoaded
          ...
          : Column(
              children: [
                PickerWidget(
                  screenType: ScreenType.reportssScreen, // adjust if needed
                  showCompareDateFilter: false,
                  showStoreFilter: true, // we have store id
                  onlyDayPicker: false,
                  getDate: (dt1, dt2, dt3, dt4, min, max, bill) async {
                    _date = dt2; // using end date
  ...
  Widget _buildRow(ProductRestResponseModel item, int index) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 14),
      decoration: BoxDecoration(
        border: Border(
