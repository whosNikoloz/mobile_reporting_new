...
enum _ReportFilterType { sales, quantity }
...
class _TopProductsScreenState extends State<TopProductsScreen> {
  ...
  // Data
  List<TopProductSalesResponseModel> _reportData = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final response = await _reportsService.getTopProductsReport(
      ...
      print('❌ Error loading top products: $err');
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.grey[50], // Same bg as StoreSalesScreen
  ...
  List<PieChartSectionData> _buildPieChartSections() {
    ...
    return List.generate(_reportData.length, (i) {
      final isTouched = false; // Add touch support if needed
      final fontSize = isTouched ? 16.0 : 12.0;
      final radius = isTouched ? 60.0 : 50.0;
      final item = _reportData[i];
      final value = _getMetricValue(item, isCurrent: true);
  ...
  void _showDisplayValueSelector() {
